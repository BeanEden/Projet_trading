"""
T08 v6 – RL avec Features Temporelles & Données Étendues
Entraînement sur 2022+2023+2024 (3 ans), Validation sur 2025.
Env V6 (Signal + Auto TP/SL + Time Features).

Usage :
  python training/train_rl_v6.py
"""
...
MODEL_DIR = PROJECT_ROOT / "models" / "v6"
MODEL_DIR.mkdir(parents=True, exist_ok=True)

RL_CONFIG = {
    "algorithm": "PPO",
    "total_timesteps": 1_000_000,   # Plus de données -> Plus de steps
    "learning_rate": 2e-4,
    "gamma": 0.995,
    "batch_size": 512,
    "n_steps": 4096,
    ...
    "gae_lambda": 0.95,
    "seed": 42,
    "window_size": 48,              # Fenêtre plus large pour capter cycles jour
    "spread": 0.00015,
    "take_profit_pct": 0.003,       # 0.3% TP
    "stop_loss_pct": 0.002,         # 0.2% SL
    "max_hold": 48,
    "cooldown": 4,
    "net_arch": [256, 128, 64],     # Réseau plus profond
}


class EvalCallback(BaseCallback):
    def __init__(self, eval_env, eval_freq=50_000, save_path=None):
...
def train():
    print("=" * 60)
    print("T08 v6 - RL Training (2022-2024) with Time Features")
    print("=" * 60, flush=True)

    # Train on 2022, 2023, AND 2024
    ...
    # Callback on 2025 data every 50k steps
    callback = EvalCallback(eval_env=env_test, eval_freq=50_000, save_path=MODEL_DIR)
    ...
    results = {}
    for year, label in [(2022, "2022"), (2023, "2023"), (2024, "2024"), (2025, "2025 (Test)")]:
